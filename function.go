...
func HTTPFunction(_w http.ResponseWriter, _r *http.Request) {
	...
	tweets := getTweetsByList(1375784169129738240)
	var text string
...
func getTweetsByList(listId int64) []twitter.Tweet {
	...
	members, resp, err := client.Lists.Members(&twitter.ListsMembersParams{ListID: listId, Count: 1000})
	if err != nil {
	...
	if resp.StatusCode == 200 {
		for _, user := range members.Users {
			...
			if resp.StatusCode == 200 {
				response = append(response, tweets...)
			}
		}
	}
	return response
